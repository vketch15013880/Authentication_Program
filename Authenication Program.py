#Requirments:
#Username and Password: (Create several username and password)
#Place to login
#Store the Username and Password(Dictionary)
#No 2 users have the same username
#Password Complexity Requirments (maximum password(69 chars), 6 chars, special char($,_,-,.,),1 num,
#1 lowercase, 1 uppercase
#Check the password entered (type the password and retype it)

#a page that directs users to make username or login or exit the program





#Making a username and password

class Creatingaccountpage:
  def store():
    logins={}

    username= input("Create a username:")
    password=input("Create a password:")

    logins[username] = password    
    for login in logins:
        print (logins)

  def store():
    store()
    
    
  if __name__ == "__main__":
    
  main()print("Account Creation")
  print("At any time enter menu to return to the menu page")
  username = input("Enter a username:  ")
  for username in set(store.values()):
  


#password recheck





#Login 






#after recieving validation they are given an option of returning to main menu



















